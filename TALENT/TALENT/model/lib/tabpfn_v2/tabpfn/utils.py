...
from typing import TYPE_CHECKING, Any, Literal, overload, Union, Optional
...
def infer_device_and_type(device: Union[str, torch.device, None]) -> torch.device:
...
def infer_fp16_inference_mode(device: torch.device, *, enable: Optional[bool]) -> bool:
    ...
    if enable is True:
        if not fp16_available:
            raise ValueError(
                ...
                " are compatible with torch.amp.autocast()`"
...
def load_model_criterion_config(
    ...
    *,
    ...
    PerFeatureTransformer,
    Union[nn.BCEWithLogitsLoss, nn.CrossEntropyLoss, FullSupportBarDistribution],
    InferenceConfig,
]:
...
def _fix_dtypes(
    ...
    cat_indices: Optional[Sequence[Union[int, str]]],
...
) -> tuple[np.ndarray, np.ndarray, Optional[npt.NDArray[Any]], int]:
...
def infer_categorical_features(
    X: np.ndarray,
    *,
    provided: Optional[Sequence[int]],
...
def infer_random_state(
    random_state: Union[int, np.random.RandomState, np.random.Generator, None],
...
def update_encoder_outlier_params(
    model: nn.Module,
    remove_outliers_std: Optional[float],
    seed: Optional[int],
    *,
...
def _transform_borders_one(
    borders: np.ndarray,
    target_transform: Union[TransformerMixin, Pipeline],
    *,
    repair_nan_borders_after_transform: bool,
) -> tuple[Optional[npt.NDArray[np.bool_]], bool, np.ndarray]:
    ...
    logit_cancel_mask: Optional[npt.NDArray[np.bool_]] = None
