...
def initialize_tabpfn_model(
    ...
    fit_mode: Literal["low_memory", "fit_preprocessors", "fit_with_cache"],
    static_seed: int,
) -> tuple[PerFeatureTransformer, InferenceConfig, Optional[FullSupportBarDistribution]]:
...
def determine_precision(
    inference_precision: Union[torch.dtype, Literal["autocast", "auto"]],
    device_: torch.device,
) -> tuple[bool, Optional[torch.dtype], int]:
...
def create_inference_engine(  # noqa: PLR0913
    *,
    ...
    n_jobs: int,
    byte_size: int,
    forced_inference_dtype_: Optional[torch.dtype],
